@if ($paginator->hasPages())
    <nav role="navigation" aria-label="Pagination Navigation" class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div class="inline-flex items-center gap-2 rounded-[1.25rem] border border-[#004777]/10 bg-white/95 p-2 shadow-[0_18px_45px_-28px_rgba(0,71,119,0.35)]">
            ...
            @else
                <a href="{{ $paginator->previousPageUrl() }}" rel="prev" class="inline-flex min-w-[2.75rem] items-center justify-center rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-[#004777] transition hover:-translate-y-0.5 hover:border-[#35A7FF] hover:bg-[#eef8ff]">
                    Prev
                </a>
            @endif
                ...
                @foreach ($elements as $element)
                    ...
                    @if (is_array($element))
                        @foreach ($element as $page => $url)
                            ...
                            @else
                                <a href="{{ $url }}" class="inline-flex min-w-[2.75rem] items-center justify-center rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-[#004777] transition hover:-translate-y-0.5 hover:border-[#35A7FF] hover:bg-[#eef8ff]">
                                    {{ $page }}
                                </a>
                            @endif
                        @endforeach
            ...
            @if ($paginator->hasMorePages())
                <a href="{{ $paginator->nextPageUrl() }}" rel="next" class="inline-flex min-w-[2.75rem] items-center justify-center rounded-xl bg-[#35A7FF] px-3 py-2 text-sm font-semibold text-white transition hover:-translate-y-0.5 hover:bg-[#1d9bf0]">
                    Next
                </a>
            @else
                <span class="inline-flex min-w-[2.75rem] items-center justify-center rounded-xl border border-slate-200 bg-slate-100 px-3 py-2 text-sm font-semibold text-slate-400">
