<div x-data="{ open: @entangle('showModal').live, deleteOpen: @entangle('showDeleteModal').live }" class="space-y-6">
    <x-ui.page-header title="{{ __('admin.permissions.page_title') }}" subtitle="{{ __('admin.permissions.page_subtitle') }}">
        <button wire:click="create" class="rounded-xl bg-slate-900 px-4 py-2 text-sm text-white transition hover:bg-slate-800">
    ...
    <section class="space-y-4">
        <div class="rounded-2xl border bg-white p-4">
            <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
                <input wire:model.live="search" class="w-full rounded-xl border px-4 py-2" placeholder="{{ __('admin.permissions.search_placeholder') }}">

                <select wire:model.live="perPage" class="w-full rounded-xl border px-4 py-2">
                    <option value="5">{{ trans_choice('admin.permissions.per_page', 5) }}</option>
                    <option value="10">{{ trans_choice('admin.permissions.per_page', 10) }}</option>
                    <option value="20">{{ trans_choice('admin.permissions.per_page', 20) }}</option>
                </select>
            </div>
        </div>
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex justify-end gap-2">
                                        <div class="relative group">
                                            <button wire:click="edit('{{ $row->id }}')" class="inline-flex h-9 w-9 items-center justify-center rounded-lg bg-blue-50 text-blue-600 transition hover:bg-blue-100" title="{{ __('admin.permissions.actions.edit') }}">
                                            ...
                                            <span class="pointer-events-none absolute -top-9 left-1/2 -translate-x-1/2 whitespace-nowrap rounded-md bg-slate-900 px-2 py-1 text-[11px] font-medium text-white opacity-0 transition group-hover:opacity-100 group-focus-within:opacity-100">
                                                {{ __('admin.permissions.actions.edit') }}
                                            </span>
                                        </div>

                                        <div class="relative group">
                                            <button wire:click="confirmDelete('{{ $row->id }}')" class="inline-flex h-9 w-9 items-center justify-center rounded-lg bg-rose-50 text-rose-600 transition hover:bg-rose-100" title="{{ __('admin.permissions.actions.delete') }}">
                                                <span class="sr-only">{{ __('admin.permissions.actions.delete') }}</span>
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor" class="h-4 w-4">
        ...
        <div x-show="open" x-cloak class="fixed inset-0 z-50 flex items-center justify-center">
            ...
            <div @click.stop
                ...
                <div class="mt-6 flex justify-end gap-2">
                    ...
                    <button wire:click="save" class="rounded-xl bg-slate-900 px-4 py-2 text-white hover:bg-slate-800">
        ...
        <div x-show="deleteOpen" x-cloak class="fixed inset-0 z-50 flex items-center justify-center">
            ...
            <div @click.stop
                ...
                <p class="mt-2 text-sm text-slate-600">
                    {{ __('admin.permissions.delete.subtitle') }}
                </p>
